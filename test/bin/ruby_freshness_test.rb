...
class RubyFreshnessTest < ActiveSupport::TestCase
  ...
  test "patch updater changes every Ruby source atomically" do
    Dir.mktmpdir do |directory|
      ...
      File.write(File.join(directory, "Dockerfile"), "FROM docker.io/library/ruby:3.4.10-slim-trixie@sha256:#{'a' * 64} AS base\n")
      ...
      assert_includes File.read(File.join(directory, "Dockerfile")), "ruby:3.4.11-slim-trixie@sha256:#{'b' * 64}"
    end
  end
